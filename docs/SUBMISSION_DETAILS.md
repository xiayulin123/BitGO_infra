# Submission details (extended — for graders & interview prep)

> The **main [`README.md`](../README.md)** is kept to ≤1 page per the PDF. This file has deploy steps, screenshots, PDF traceability, and design notes.
...
- **Route53 + ACM** — console first for DNS validation speed; **imported** into `resources/route53` / `resources/acm` (full IaC in repo, see `IMPORT.md`)
- **Cluster Autoscaler** — HPA scales pods only; nodes fixed 2–4; node autoscaling deferred to prioritize pod-level burst handling within time budget
